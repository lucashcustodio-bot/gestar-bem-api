...
def make_cover(idx, slide, cfg):
    img = Image.new("RGB",(W,H),BG); d = ImageDraw.Draw(img)
    bg_grad(d,740,PURPLE2,0.7)
...
def make_emotional(idx, slide, cfg):
    img = Image.new("RGB",(W,H),BG); d = ImageDraw.Draw(img)
    bg_grad(d,600,PURPLE2,0.6)
    sidebar(d,LILAC,PURPLE)
    img = draw_circles(img,get_circles(idx)); d = ImageDraw.Draw(img)
    ...
    lines_h = label.split("|") if "|" in label else label.split(",")
    for i,ln in enumerate(lines_h[:3]):
        col = PURPLE if i==len(lines_h[:3])-1 else CREAM
...
def make_cta(idx, slide, cfg):
    img = Image.new("RGB",(W,H),BG); d = ImageDraw.Draw(img)
    bg_grad(d,440,PURPLE2,0.78)
    sidebar(d,LILAC,PURPLE)
    img = draw_circles(img,get_circles(idx)); d = ImageDraw.Draw(img)
    d.line([(54,82),(W-54,82)],fill=(42,32,52),width=1)
    d.text((54,44),"G E S T A R   B E M",font=pp(22,"Light"),fill=DIM)
    ...
    if resto:
        y = wrp(d,resto,pp(33,"Light"),x,y,W-120,DIM,1.46); y+=28
    ctas = ["Salva este post para os dias difíceis.",
            "Comenta aqui: você já sabia disso? 👇"]
    for txt in ctas:
...
def gerar():
    ...
    try:
        ...
        for i, slide in enumerate(slides):
            if i == 0:
                img = make_cover(i, slide, cfg)
            elif i == n-1:
                img = make_cta(i, slide, cfg)
            elif i == n-2:
                img = make_emotional(i, slide, cfg)
            else:
                img = make_light(i, slide, cfg)
        ...
        return send_file(buf, mimetype="application/zip",
                         as_attachment=True,
                         download_name="gestar-bem-carrossel.zip")
